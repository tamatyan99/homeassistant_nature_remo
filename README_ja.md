...
- 温度・湿度・相対照度（0–200スケール）・気圧・人感センサーのデータ取得
...
- ローカルIP設定（将来のローカルAPI機能用；現状はすべてクラウドAPI経由）
...
- データの更新間隔を選択肢から選べます（秒）
  - 選択肢: `30`, `60`, `90`（デフォルト: `60`）
- 人感検出の閾値を選択肢から選べます（分）
  - 選択肢: `1`, `3`, `5`, `10`, `15`（デフォルト: `5`）
- Nature RemoのローカルIPアドレスを設定できます（将来の機能用）。
...
| 種類          | 説明                                                              |
|---------------|-------------------------------------------------------------------|
| climate       | エアコンの操作（冷房・暖房・除湿・送風・自動、ecoプリセット）    |
| light         | 照明の操作（オン／オフ、モード切替、エフェクト）                  |
| sensor        | 温度、湿度、相対照度（0–200スケール）、気圧、電力（買電／売電／瞬時電力）、人感タイムスタンプ |
| remote        | IR／AC／LIGHTの signals 送信、および電源オン／オフ操作           |
| switch        | 電源signalsを持つ家電のオン／オフ切替                             |
| binary_sensor | 検出閾値を設定可能な人感センサー                                  |
| event         | モーション検知イベント（オートメーション連携用）                   |
| button        | IR信号学習、データ更新ボタン                                       |
| select        | 照明モード・エアコンプリセット選択UI                               |
| diagnostics   | 診断情報のダウンロード                                             |
...
data:
  command: "power on"  # Remoに登録されたボタン名（大文字小文字は区別されません）
```
...
注意: これらの値は表示専用です。エアコンの制御コマンドは、Nature Remo APIへデバイス自身のセンサー値を用いて送信されます。
