...
class TestNatureRemoAPI:
    ...
    async def test_call_api_retries_get_on_timeout_then_succeeds(self, api):
    ...
    async def test_call_api_does_not_retry_post_on_timeout(self, api):
        class TimeoutCtx:
            async def __aenter__(self):
                raise TimeoutError()
            async def __aexit__(self, *args):
    ...
    async def test_call_api_does_not_retry_on_429_for_post(self, api):
        resp_429 = _mock_response(status=429)
