...
class TestNatureRemoAPI:
    ...
    async def test_call_api_retries_on_timeout_when_enabled_then_succeeds(self, api):
        ...
        result = await api._call_api("GET", "/devices", max_retries=1)
        assert result == {"status": "ok"}
        assert api._session.request.call_count == 2

    async def test_call_api_does_not_retry_post_on_429_by_default(self, api):
        resp_429 = _mock_response(status=429)
    ...
    async def test_send_command_signal_does_not_retry_on_timeout(self, api):
        resp_200 = _mock_response(status=200, json_data={"status": "ok"})
